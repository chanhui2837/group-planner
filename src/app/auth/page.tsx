    ...
    <div className="min-h-screen flex bg-[#FFF8F0]">
      <div className="hidden lg:flex flex-1 relative overflow-hidden bg-gradient-to-br from-[#FF6B6B] via-[#FF8E53] to-[#FFE66D] p-10 flex-col justify-between">
        ...
        <div className="flex items-center gap-2 text-white/80 text-xs font-bold">
          ...
          실시간 MongoDB 동기화 • 최대 10명 그룹 • 이메일 다계정 허용
        </div>
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="bg-white rounded-[28px] shadow-[0_16px_40px_rgba(255,107,107,0.12)] border border-[#FFE0CC] p-7">
            <div className="flex gap-2 p-1 bg-[#FFF0E6] rounded-2xl w-fit">
              <button onClick={() => setMode("login")} className={`px-6 py-2 rounded-xl text-sm font-black transition ${mode === "login" ? "bg-[#FF6B6B] text-white shadow" : "text-[#636E72]"}`}>로그인</button>
              <button onClick={() => setMode("signup")} className={`px-6 py-2 rounded-xl text-sm font-black transition ${mode === "signup" ? "bg-[#FF6B6B] text-white shadow" : "text-[#636E72]"}`}>회원가입</button>
            </div>

            <h2 className="mt-6 text-[22px] font-black text-[#2D3436] leading-tight">{mode === "login" ? "다시 만나서 반가워요! 👋" : "가족을 초대해볼까요? 🏡"}</h2>
            <p className="text-sm text-[#636E72] mt-1">{mode === "login" ? "아이디와 비밀번호로 로그인하세요." : "실명, 아이디, 비밀번호, 이메일로 바로 가입해요. 같은 이메일로 여러 계정 가능!"}</p>

            <form onSubmit={submit} className="mt-6 space-y-4">
              {mode === "signup" && (
                <div>
                  <label className="text-xs font-bold text-[#636E72]">실명</label>
                  <input value={form.realName} onChange={(e) => setForm({ ...form, realName: e.target.value })} placeholder="홍길동" required className="mt-1 w-full px-4 py-3 rounded-2xl bg-[#FFF8F0] border border-[#FFE0CC] focus:outline-none focus:ring-2 focus:ring-[#FF6B6B]/20 focus:border-[#FF6B6B] text-sm" />
                </div>
              )}
              <div>
                <label className="text-xs font-bold text-[#636E72]">아이디</label>
                <input value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="family123" required className="mt-1 w-full px-4 py-3 rounded-2xl bg-[#FFF8F0] border border-[#FFE0CC] focus:outline-none focus:ring-2 focus:ring-[#FF6B6B]/20 focus:border-[#FF6B6B] text-sm" />
              </div>
              {mode === "signup" && (
                <div>
                  <label className="text-xs font-bold text-[#636E72]">이메일 <span className="text-[#00B894] text-[11px]">· 같은 이메일로 여러 아이디 가능</span></label>
                  <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" required className="mt-1 w-full px-4 py-3 rounded-2xl bg-[#FFF8F0] border border-[#FFE0CC] focus:outline-none focus:ring-2 focus:ring-[#FF6B6B]/20 focus:border-[#FF6B6B] text-sm" />
                </div>
              )}
              <div>
                <label className="text-xs font-bold text-[#636E72]">비밀번호</label>
                <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" required className="mt-1 w-full px-4 py-3 rounded-2xl bg-[#FFF8F0] border border-[#FFE0CC] focus:outline-none focus:ring-2 focus:ring-[#FF6B6B]/20 focus:border-[#FF6B6B] text-sm" />
              </div>
              ...
              <button type="submit" disabled={loading} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] text-white font-black shadow-[0_8px_20px_rgba(255,107,107,0.35)] hover:shadow-[0_12px_28px_rgba(255,107,107,0.45)] transition disabled:opacity-60">
                {loading ? "처리 중..." : mode === "login" ? "로그인하기 →" : "계정 만들고 시작하기 ✨"}
              </button>

              <div className="text-center text-xs text-[#B2BEC3]">MongoDB에 모든 정보가 실시간으로 저장돼요. <span className="text-[#FF6B6B] font-bold">안전하게 암호화</span> 됩니다.</div>
            </form>
          </div>

          <div className="mt-4 text-center text-xs text-[#636E72]">같은 이메일로 여러 계정을 바로 만들 수 있어요 — 인증 없이 즉시 사용.</div>
        </div>
      </div>
      ...
      {showFindId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[24px] w-full max-w-[420px] p-6">
            ...
            <p className="text-xs text-[#636E72] mt-1">가입한 이메일로 아이디를 찾아요</p>
            ...
            <button onClick={handleFindId} disabled={findLoading} className="mt-3 w-full py-3 rounded-xl bg-[#4ECDC4] text-white font-black text-sm disabled:opacity-50">{findLoading ? "조회 중..." : "아이디 찾기"}</button>
            {findResult && (
          ...
          <div className="relative bg-white rounded-[24px] w-full max-w-[420px] p-6 max-h-[90vh] overflow-y-auto">
            ...
            {resetStep === "request" ? (
              <>
                ...
                <button onClick={handleRequestReset} disabled={findLoading} className="mt-3 w-full py-3 rounded-xl bg-[#FF6B6B] text-white font-black text-sm disabled:opacity-50">{findLoading ? "발송 중..." : "인증코드 발송"}</button>
              </>
            ) : (
              <>
                ...
                <button onClick={handleResetPw} disabled={findLoading} className="mt-3 w-full py-3 rounded-xl bg-[#00B894] text-white font-black text-sm disabled:opacity-50">{findLoading ? "변경 중..." : "비밀번호 변경"}</button>
